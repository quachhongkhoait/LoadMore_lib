...
import com.chanbedu.loadmorelib.ext.invisible
import com.chanbedu.loadmorelib.ext.isVisible
import com.chanbedu.loadmorelib.ext.safeClick
import com.chanbedu.loadmorelib.ext.visible

class LoadMoreAdapter internal constructor(adapter: RecyclerView.Adapter<*>) :
    RecyclerView.Adapter<RecyclerView.ViewHolder>() {

    var progressView: View? = null
    var progressResId = View.NO_ID

    var reachEndView: View? = null
    var reachEndResId = View.NO_ID

    var loadFailedView: View? = null
    var loadFailedResId = View.NO_ID
    ...
    private val onScrollListener = object : RecyclerView.OnScrollListener() {
        override fun onScrollStateChanged(recyclerView: RecyclerView, newState: Int) {
            ...
            if (!loadMoreEnabled || isLoading ||
                onLoadMoreListener == null || newState != RecyclerView.SCROLL_STATE_IDLE
            ) {
                return
            }

            val isBottom = when (val layoutManager = recyclerView.layoutManager) {
                ...
                is StaggeredGridLayoutManager -> {
                    ...
                    val lastVisiblePosition = (layoutManager as GridLayoutManager)
    ...
    var loadMoreEnabled: Boolean = true
    ...
    override fun onBindViewHolder(
        ...
        when (holder) {
            is ProgressHolder -> {
                // FIXME: Check canScroll condition if something went wrong.
                if (canScroll() && onLoadMoreListener != null && !isLoading && itemCount >= visibleThreshold) {
                    isLoading = true
    ...
    private fun canScroll(): Boolean {
        if (recyclerView == null) {
            throw NullPointerException("recyclerView is null, you should setAdapter(recyclerAdapter);")
        }
        val layoutManager = recyclerView?.layoutManager
        if (layoutManager is LinearLayoutManager) {
            return layoutManager.findLastCompletelyVisibleItemPosition() < originalAdapter.itemCount - 1
        }
    ...
    private fun notifyFooterHolderChanged() {
        if (loadMoreEnabled) {
            ...
            shouldRemove = false
            /*
              fix IndexOutOfBoundsException when setLoadMoreEnabled(false), then use onItemRangeInserted
              @see validateViewHolderForOffsetPosition(RecyclerView.ViewHolder)
             */
    ...
    private class ProgressHolder constructor(itemView: View) : RecyclerView.ViewHolder(itemView) {
        init {
        ...
        fun bind(isVisible: Boolean) {
            ...
            if (isVisible) itemView.visible() else itemView.invisible()
        }
    }

    private class ReachEndHolder constructor(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    private class LoadFailedHolder constructor(
        itemView: View,
    ...
    interface OnLoadMoreListener {
        fun onPrepared(controller: LoadMoreController?)
